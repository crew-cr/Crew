...
/**
 * Base class that represents a row from the 'line_comment' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseLineComment extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'LineCommentPeer';
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = LineCommentPeer::ID;
		}
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = LineCommentPeer::USER_ID;
		}
	...
	public function setFileId($v)
	{
		...
		if ($this->file_id !== $v) {
			$this->file_id = $v;
			$this->modifiedColumns[] = LineCommentPeer::FILE_ID;
		}
	...
	public function setPosition($v)
	{
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns[] = LineCommentPeer::POSITION;
		}
	...
	public function setLine($v)
	{
		...
		if ($this->line !== $v) {
			$this->line = $v;
			$this->modifiedColumns[] = LineCommentPeer::LINE;
		}
	...
	public function setValue($v)
	{
		...
		if ($this->value !== $v) {
			$this->value = $v;
			$this->modifiedColumns[] = LineCommentPeer::VALUE;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = LineCommentPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseLineComment:delete:pre') as $callable)
			{
			...
			if ($ret) {
				LineCommentQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseLineComment:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseLineComment:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseLineComment:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				LineCommentPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(LineCommentPeer::ID) ) {
					...
					$affectedRows += LineCommentPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = LineCommentPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = LineCommentPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = LineCommentPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	{
		$criteria = new Criteria(LineCommentPeer::DATABASE_NAME);
		$criteria->add(LineCommentPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of LineComment (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     LineComment Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     LineCommentPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new LineCommentPeer();
		}
		return self::$peer;
	}
	...
	public function getsfGuardUser(PropelPDO $con = null)
	{
		if ($this->asfGuardUser === null && ($this->user_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 $this->asfGuardUser->addLineComments($this);
			 */
		}
	...
	public function getFile(PropelPDO $con = null)
	{
		if ($this->aFile === null && ($this->file_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 undesirable since it could result in an only partially populated collection
				 in the referenced object.
				 $this->aFile->addLineComments($this);
			 */
		}
		return $this->aFile;
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseLineComment:' . $name))
		{

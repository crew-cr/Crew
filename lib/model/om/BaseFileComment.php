...
/**
 * Base class that represents a row from the 'file_comment' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseFileComment extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'FileCommentPeer';
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = FileCommentPeer::ID;
		}
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = FileCommentPeer::USER_ID;
		}
	...
	public function setFileId($v)
	{
		...
		if ($this->file_id !== $v) {
			$this->file_id = $v;
			$this->modifiedColumns[] = FileCommentPeer::FILE_ID;
		}
	...
	public function setValue($v)
	{
		...
		if ($this->value !== $v) {
			$this->value = $v;
			$this->modifiedColumns[] = FileCommentPeer::VALUE;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = FileCommentPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseFileComment:delete:pre') as $callable)
			{
			...
			if ($ret) {
				FileCommentQuery::create()
					->filterByPrimaryKey($this->getPrimaryKey())
					->delete($con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseFileComment:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseFileComment:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseFileComment:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				FileCommentPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FileCommentPeer::ID) ) {
					...
					$affectedRows += FileCommentPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = FileCommentPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = FileCommentPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = FileCommentPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FileCommentPeer::ID)) $criteria->add(FileCommentPeer::ID, $this->id);
	...
	{
		$criteria = new Criteria(FileCommentPeer::DATABASE_NAME);
		$criteria->add(FileCommentPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of FileComment (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     FileComment Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     FileCommentPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new FileCommentPeer();
		}
		return self::$peer;
	}
	...
	public function getsfGuardUser(PropelPDO $con = null)
	{
		if ($this->asfGuardUser === null && ($this->user_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 $this->asfGuardUser->addFileComments($this);
			 */
		}
	...
	public function getFile(PropelPDO $con = null)
	{
		if ($this->aFile === null && ($this->file_id !== null)) {
			...
			/* The following can be used additionally to
				 ...
				 undesirable since it could result in an only partially populated collection
				 in the referenced object.
				 $this->aFile->addFileComments($this);
			 */
		}
		return $this->aFile;
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseFileComment:' . $name))
		{

...
abstract class BaseRequest extends BaseObject  implements Persistent
{
	 ...
	 * @return     Request The current object (for fluent API support)
	 */
	...
	public function setBranchId($v)
	{
		...
		if ($this->branch_id !== $v) {
			$this->branch_id = $v;
			$this->modifiedColumns[] = RequestPeer::BRANCH_ID;
		}
	...
	public function setCommit($v)
	{
		...
		if ($this->commit !== $v) {
			$this->commit = $v;
			$this->modifiedColumns[] = RequestPeer::COMMIT;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = RequestPeer::CREATED_AT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = RequestPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RequestPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = RequestQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRequest:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRequest:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRequest:save:pre') as $callable)
			{
			...
			if ($isInsert) {
				...
				if (!$this->isColumnModified(RequestPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRequest:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RequestPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(RequestPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(RequestPeer::BRANCH_ID)) {
			$modifiedColumns[':p' . $index++]  = '`BRANCH_ID`';
		}
		if ($this->isColumnModified(RequestPeer::COMMIT)) {
	...
	{
		$pos = RequestPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Request'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Request'][$this->getPrimaryKey()] = true;
		$keys = RequestPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RequestPeer::BRANCH_ID)) $criteria->add(RequestPeer::BRANCH_ID, $this->branch_id);
		if ($this->isColumnModified(RequestPeer::COMMIT)) $criteria->add(RequestPeer::COMMIT, $this->commit);
		if ($this->isColumnModified(RequestPeer::CREATED_AT)) $criteria->add(RequestPeer::CREATED_AT, $this->created_at);
	...
	{
		return (string) $this->exportTo(RequestPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseRequest:' . $name))
		{
